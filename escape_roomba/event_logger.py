...
class EventLogger:
    ...
    async def _on_resumed(self):
        logger.info(f'Resumed Discord session')

    async def _on_ready(self):
        c = self._discord
        logger.info(f'Ready in {len(c.guilds)} servers (as {c.user}):' +
                    ''.join(f'\n    "{g}"' for g in c.guilds))
        invite_url = discord.utils.oauth_url(
            ...
            discord.Permissions(
                ...
                manage_roles=True))
        logger.info('Open this link to add this bot to more servers:\n'
                    f'    {invite_url}')

    async def _on_guild_join(self, guild):
        logger.info(f'Joined Discord guild (server) "{guild}"')

    async def _on_guild_remove(self, guild):
        logger.info(f'Removed from Discord guild (server) "{guild}"')

    async def _on_error(self, event, *args, **kwargs):
        logger.exception(f'Exception in "{event}" handler:')
